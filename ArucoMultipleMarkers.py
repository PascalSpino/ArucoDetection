...
while True:
    ...
    if len(ids) > 0:
        ...
        ids = ids.flatten()
        # print(list(zip(corners[0], ids)))

        # loop over the detected ArUCo corners
        for (markerCorner, markerID) in zip(corners, ids):
            # capturing rotation and translation vectors for detected marker
            rvec, tvec, objPts = cv2.aruco.estimatePoseSingleMarkers(markerCorner, markerLength,
                                                                     newcameramtx, dist)
            ...
            # draw the distance to the marker on the frame
